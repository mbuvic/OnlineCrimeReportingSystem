...
<?php
	...
	if($status > 1){
		...
		if ($aqry && $aqry->num_rows > 0) { // Check if there are rows
			$ca = $aqry->fetch_array(); // Fetch the row as an associative array
		} else {
			$ca = []; // Initialize as empty array to avoid undefined variable issues
		}
	}	
			...
			<div class="assign-responder" style="display:none">
				<div class="form-group">
					...
					<select name="responder_id" id="" class="custom-select input-sm select2">
						...
						?>
						<option value="<?php echo $row['id'] ?>" <?php echo isset($ca['responder_id']) && $ca['responder_id'] == $row['id'] ? 'selected' : '' ?>><?php echo $row['sname'].' - '. $row['name'] ?></option>
						<?php endwhile; ?>
					</select>
				</div>
			</div>
